...
with st.expander("ℹ️ ¿Cómo usar esta simulación?"):
    ...
    **Parámetros principales:**
    - **`g` (productividad de investigación)**: Qué tan eficaz es cada persona generando innovación.  
      Valores típicos: entre **0.0001 y 0.002**. Si es muy bajo, el crecimiento es lento; si es muy alto, explota antes de 1950.
...
ax2.plot(P_plot, gr_plot, label="Simulación (hasta 1950)", color="red")

# Añadir regresión lineal sin intercepto (como en Kremer)
if len(P_plot) > 1 and len(gr_plot) > 1:
    X = P_plot
    y = gr_plot
